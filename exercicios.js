...
function real(a){
    parseFloat(a)
    let texto = String(a.toFixed(2))
    return `R$${texto.replace('.',',')}`
}
console.log(real(3.2)) */

/* // 06) Elabore duas funções que recebem três parâmetros: capital inicial, taxa de juros e tempo de aplicação. A
// primeira função retornará o montante da aplicação financeira sob o regime de juros simples e a segunda
// retornará o valor da aplicação sob o regime de juros compostos.
function simples(ci, tj, t){
    return ci+ (ci * tj/100  * t)
}
function compostos(ci, tj, t){
    return (ci*(Math.pow(1+tj/100, t)))
}
console.log(simples(400,5,4))
console.log(compostos(1400,7,2)) */
...
while(true){
    if(valor>=nota){
        ...
        if(qtd!==0){console.log(`${qtd} nota(s) de R${nota}`)}
        if(nota==100){nota=50}
        else if(nota==50){nota=10}
